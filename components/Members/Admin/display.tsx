...
const BlogList = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      ...
      setBlogs(blogsList);  // No need for @ts-ignore now
  ...
  const handleDelete = async (id: string) => { // id should be a string
    try {
  ...
  const renderSubBlogs = (parentBlogId: string | null) => {
    return blogs
      ...
      .map(subBlog => (
        <div key={subBlog.id} className="ml-5 border-l-2 border-gray-200 pl-4">
          ...
          {renderSubBlogs(subBlog.id)} {/* Recursive call for sub-blogs */}
        </div>
      ));
  };
